"""Hash-addressed relay for Phase-2 K-RVEA proposals on ``coconutg2``.

The Princess-side controller owns campaign state and CST observations.  This
module sends only compact numeric arrays to the proposal worker, fits the two
expensive Gaussian-process models in float64 on the requested device,
...
Every objective uses minimization semantics in this order::
    ...
    [worst |S11|, -ROI theta radiation gain dBi, normalized area]

The ROI gain is evaluated at 3.6 GHz over theta=55..85 deg and
phi=90+/-50 deg, averaged in linear power, and converted to dBi by
:mod:`phase2_krvea_data`.
"""
...
EXPENSIVE_OBJECTIVE_INDICES = (0, 1)
...
SURROGATE_TARGET_TRANSFORMS = ("logit", "identity")
...
class SurrogateFitSettings:
    """Numerical settings for the two independent batched GPs."""
    ...
    uncertainty_calibration_factors: tuple[float, float] = (1.0, 1.0)
    uncertainty_calibration_source: str = (
        "phase2_roi_radiation_gain_initial_uncalibrated"
    )
...
class ObjectiveScaler:
    """Robust physical-to-model scaling for the three objectives."""
    ...
    def __post_init__(self) -> None:
        ...
        if center.shape != expected or scale.shape != expected or penalty.shape != expected:
            raise ValueError("objective scaler arrays must have shape (3,)")
        if not (
...
class SurrogateTargetScaler:
    """Model-space transform for the two expensive objectives."""
    ...
    def __post_init__(self) -> None:
        ...
        if center.shape != expected or scale.shape != expected:
            raise ValueError("surrogate target arrays must have shape (2,)")
    ...
    def _forward_physical(self, values: np.ndarray) -> np.ndarray:
        ...
        if physical.ndim != 2 or physical.shape[1] != len(
            ...
            raise ValueError("expensive objective array must have shape (n, 2)")
        ...
        if np.any(bounded < -tolerance) or np.any(bounded > 1.0 + tolerance):
            raise ValueError("S11 target must lie in [0, 1]")
        transformed = physical.copy()
        bounded = np.clip(bounded, self.epsilon, 1.0 - self.epsilon)
        transformed[:, :1] = np.log(bounded) - np.log1p(-bounded)
        return transformed
    ...
    def inverse_prediction(
        ...
        *,
        ...
        if model_mean.shape[1] != len(EXPENSIVE_OBJECTIVE_INDICES):
            raise ValueError("surrogate prediction must have two columns")
        ...
        samples = latent_mean[:, :1, None] + (
            math.sqrt(2.0) * latent_std[:, :1, None] * nodes[None, None, :]
        )
        ...
        physical_mean[:, :1] = bounded_mean
        bounded_std = np.sqrt(
            np.maximum(bounded_second_moment - bounded_mean**2, 0.0)
        )
        physical_std[:, :1] = np.where(
            latent_std[:, :1] <= np.finfo(np.float64).eps,
            0.0,
            bounded_std,
        )
...
def fit_objective_scaler(
    ...
    if values.ndim != 2 or values.shape[1] != len(OBJECTIVE_NAMES):
        raise ValueError("full objective array must have shape (n, 3)")
...
def _validate_training_arrays(
    ...
    if expensive.shape != (len(x), len(EXPENSIVE_OBJECTIVE_INDICES)):
        raise ValueError("y_expensive_minimize must have shape (n, 2)")
    if full.shape != (len(x), len(OBJECTIVE_NAMES)):
        raise ValueError("y_full_minimize must have shape (n, 3)")
    ...
    if not np.allclose(expensive, expected_expensive, rtol=1e-10, atol=1e-12):
        raise ValueError("expensive objectives disagree with full objective columns 0,1")
...
def build_request_payload(
    ...
    *,
    ...
    if config.n_variables != len(space.names) or config.n_objectives != 3:
        raise ValueError("K-RVEA config must match the 11-D/3-objective wire space")
    if iteration < 0:
    ...
    return {
        ...
        "objective_contract": {
            ...
            "roi_gain_note": (
                "3.6 GHz E-theta radiation gain over theta=55..85 deg and "
                "phi=90+/-50 deg; spatial linear-power average converted to dBi"
            ),
...
def run_request_payload(payload: Mapping[str, Any]) -> ProposalResult:
    ...
    physical_mean[:, 0] = np.clip(physical_mean[:, 0], 0.0, 1.0)
    physical_std[:, 2] = 0.0
