"""F5/CLI launcher for propagation active-learning batch 02."""
...
RUN_ID = "propagation-active-learning-12-batch02-001"
...
def build_princess_argv() -> list[str]:
    if not WORKLIST_CSV.is_file():
        raise FileNotFoundError(
            ...
            "scripts/simulation/prepare_propagation_active_learning_12_batch02.py"
        )
    argv = [
        "start",
        "--csv",
...
def main() -> int:
    print("[Propagation AL-02] 4 ROI + 4 link + 2 uncertainty + 2 exploit")
    print(
        "[Propagation AL-02] template: "
