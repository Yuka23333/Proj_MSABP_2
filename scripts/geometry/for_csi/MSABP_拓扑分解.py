...
CPW_FEED_SLOT_WIDE_WIDTH = 2.4  # kept only as the basis for the branch keepout margin below

CPW_KEEPOUT_MARGIN = 2  # mm, safety margin kept clear around where CPW_Feed_Slot used to be
...
def plot_geom(ax, geom, style):
    ...
    linewidth = style.pop("linewidth", None)
    polys = [geom] if geom.geom_type == "Polygon" else list(geom.geoms)
    for poly in polys:
        x, y = poly.exterior.xy
        ax.plot(x, y, color=style.get("color"), linewidth=linewidth)
        if fill:
            ax.fill(x, y, **style)


# "Substrate" is the physical PCB rectangle. "Outer" is the actual outer contour of the metal
# Patch itself (the stepped/notched polyline) -- buffered out a hair (mitre join, so right
# angles stay right angles) just so it doesn't sit exactly on top of Patch's own edge.
...
Outer_Boundary = Patch.buffer(OUTER_SUBSTRATE_GAP, join_style=JOIN_STYLE.mitre)

fig, ax = plt.subplots()
shapes = (
    (Patch, dict(alpha=0.3, color="tab:green")),
    (Substrate_Full, dict(fill=False, color="black", linewidth=BOUNDARY_LINEWIDTH)),
    (Outer_Boundary, dict(fill=False, color="dimgray", linewidth=BOUNDARY_LINEWIDTH)),
    (Slot, dict(alpha=0.6, color="tab:blue", linewidth=BOUNDARY_LINEWIDTH)),
)
for shape, style in shapes:
    plot_geom(ax, shape, style)

ax.plot(*Branch_Up_1_Midpoint.xy, "ko", markersize=6, label="Branch_Up_1_Midpoint")
ax.plot(*Branch_Up_1_Left_Endpoint.xy, "m^", markersize=6, label="Branch_Up_1_Left_Endpoint")
ax.plot(*Branch_Up_1_Right_Endpoint.xy, "m^", markersize=6, label="Branch_Up_1_Right_Endpoint")

for pad in (SMA_GND_Pad_1, SMA_GND_Pad_2):
    px, py = pad.exterior.xy
    ax.fill(px, py, facecolor="none", edgecolor="black", hatch="//")

legend_handles = [
    mlines.Line2D([], [], color="tab:blue", linewidth=BOUNDARY_LINEWIDTH, label="Inner"),
    mlines.Line2D([], [], color="dimgray", linewidth=BOUNDARY_LINEWIDTH, label="Outer"),
    mlines.Line2D([], [], color="black", linewidth=BOUNDARY_LINEWIDTH, label="Substrate"),
    mpatches.Patch(facecolor="tab:green", alpha=0.3, label="Metal"),
]
ax.legend(handles=legend_handles, loc="upper right")

ax.set_aspect("equal")
ax.set_title("merged slot, merged patch")
