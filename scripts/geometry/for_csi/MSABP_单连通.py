...
# 从严格意义上来说，MSABP的金属部分本质上是一个单连通域，且不具备内部和外部的区别。
# 除了上文提到的去除馈电网络后的拓扑分解外，另一种方法是添加0宽度的虚拟连接或缝隙，
# 随后再进行拓扑分解。这种分解不严格遵循形状本身的拓扑学定义且依赖额外约束，因此称为准拓扑分解。
Metal = merge_shapes([Patch, CPW_Feed_Pin])

print("Metal:", Metal)
print("Metal Bounds:", Metal.bounds)
...
fig, ax = plt.subplots()
shapes = (
    (Metal, dict(alpha=0.3, color="tab:green")),
...
N_PROFILE_SAMPLES = 400
x_lo, x_hi = slot_min_x, slot_max_x
profile_xs = [x_lo + i * (x_hi - x_lo) / (N_PROFILE_SAMPLES - 1) for i in range(N_PROFILE_SAMPLES)]
profile_ys = [
    slot_max_y + d if (d := ray_distance_to_upper_substrate(x)) is not None else float("nan")
    for x in profile_xs
]
ax.plot(profile_xs, profile_ys, "k--", linewidth=1, label="ray-cast profile")
